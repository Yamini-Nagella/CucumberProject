...
public class AirportTaxi extends AirportTaxiUi{
	...
	public void airporttab() throws PageObjectException {
		try {
			WebDriverSupport.click(driver, AirportTaxiUi.Airporttab, "Home Page", "Select Airport Tab", log, test);
		} catch (Exception e) {
			throw new PageObjectException(e.getMessage());
		}
	}
	public void city(String city) throws PageObjectException {
		try {
			WebDriverSupport.sendKeys(driver, AirportTaxiUi.city, "Home Page", "Select City", log, test, city);
			driver.findElement(By.id("ngb-typeahead-2-0")).click();
		} catch (Exception e) {
			throw new PageObjectException(e.getMessage());
		}
	}
	public void trip() throws PageObjectException {
		try {
			WebDriverSupport.click(driver, AirportTaxiUi.trip, "Airport Taxi Booking", "Trip type", log, test);
			driver.findElement(By.xpath(".//*[@value=\"pick_airport\"]")).click();
		} catch (Exception e) {
			throw new PageObjectException(e.getMessage());
		}
	}
	public void enterdetails(String add, String month, String day, String time) throws PageObjectException {
		try {
			WebDriverSupport.sendKeys(driver, AirportTaxiUi.address, "Airport Taxi Booking", "Address Field", log, test, add);
			Thread.sleep(2000);
			driver.findElement(By.xpath(".//*[@class='main-text']")).click();
			WebDriverSupport.click(driver, AirportTaxiUi.pickup, "Airport Taxi Booking", "Date Field", log, test);
			Thread.sleep(2000);
			while(true) {
			...
			Select ttime = new Select(driver.findElement(AirportTaxiUi.pickupat));
			ttime.selectByVisibleText(time);
		} catch (Exception e) {
			throw new PageObjectException(e.getMessage());
		}
	}
	public void selectcar() throws PageObjectException {
		try {
			WebDriverSupport.click(driver, AirportTaxiUi.book_btn, "Airport Taxi Booking", "click on select car", log, test);
		} catch (Exception e) {
			throw new PageObjectException(e.getMessage());
		}
	}
	public void verify(String vcity, String vdate) throws PageObjectException {
		try {
			String head = driver.findElement(AirportTaxiUi.vheading).getText();
			if (head.contains(vcity) && head.contains(vdate)) {
