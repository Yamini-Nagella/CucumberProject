...
public class LocalCars extends LocalCarsUi{
	...
	public void click() throws ReusableComponentException, Exception {
		WebDriverSupport.click(driver, localtab, "Home Page", "Select Local City Tab", log, test);
	}
	public void City(String src) throws ReusableComponentException, Exception {
		WebDriverSupport.sendKeys(driver, city, "Home Page", "Select City", log, test, src);
		driver.findElement(By.id("ngb-typeahead-2-0")).click();
	}
	public void details(String month, String day, String time) throws ReusableComponentException, Exception {
		...
		int count = driver.findElements(By.xpath(".//tbody[contains(@class,'ng-tns-c75-3')]/tr/td")).size();
		for(int i=0;i<count;i++) {
			
			if(driver.findElements(By.xpath(".//tbody[contains(@class,'ng-tns-c75-3')]/tr/td")).get(i).getText().contains(day)) {
				driver.findElements(By.xpath(".//tbody[contains(@class,'ng-tns-c75-3')]/tr/td")).get(i).click();
				break;
			}
		}
	...
	public void verify(String vcity, String vdate) throws IOException, PageObjectException {
		...
		if (head.contains(vcity) && head.contains(vdate)) {
			...
			ExtentLogUtilities.fail(driver, test, "failed to open local Taxi Booking Page", log);
	...
	public void select(String src, String month, String day, String stime, String vcity, String vdate) throws ReusableComponentException, Exception {
		try {
			...
			day = day.substring(0,2);
			int count = driver.findElements(By.xpath(".//tbody[@class='ng-tns-c75-3']")).size();
			for(int i=0;i<count;i++) {
				
				if(driver.findElements(By.xpath(".//tbody[@class='ng-tns-c75-3']")).get(i).getText().contains(day)) {
					driver.findElements(By.xpath(".//tbody[@class='ng-tns-c75-3']")).get(i).click();
					break;
				}
			}
			Select ttime = new Select(driver.findElement(pickupat));
